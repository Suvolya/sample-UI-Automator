...
//    }
    ...
    @Test
    fun testChangeTextActivity() {
        val packageName = MODEL_PACKAGE
        waitForPackage(packageName)

        device.findObject(By.res(packageName, "userInput")).text = textToSet3
        device.findObject(By.res(packageName, "buttonActivity")).click()

        device.wait(Until.findObject(By.res(packageName, "text")),
            500)

        val result = device.findObject(By.res(packageName, "text")).text

        Thread.sleep(3000)
        assertEquals(result, textToSet3)
    }
